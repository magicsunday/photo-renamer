...
- **Semantic duplicate detection:** Files that are the same capture but have different hashes (e.g., re-saved JPEGs) are detected as duplicates via two heuristics: (1) if all companion videos in a group share the same hash, the stills are treated as duplicates; (2) if files in the canonical's directory share the same non-zero subsecond timestamp and the same software tag, they are treated as duplicates. Cross-directory files with different software tags (e.g., Photoshop edits in a subfolder) are correctly separated via hash sub-grouping.
